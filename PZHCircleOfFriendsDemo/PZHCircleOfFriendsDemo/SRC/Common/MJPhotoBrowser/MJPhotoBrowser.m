...
- (void)createToolbar
{
    ...
    if (self.canDelete) {
        UIButton * deleteBtn = [[UIButton alloc]initWithFrame:CGRectMake(20, 0, 60, barHeight)];
//        deleteBtn.backgroundColor = [UIColor redColor];
    ...
    if (self.showDetail) {
        UIButton * detailBtn = [[UIButton alloc]initWithFrame:CGRectMake(20, 0, 100, barHeight)];
        ...
        [_toolbar addSubview:detailBtn];
        [detailBtn addTarget:self action:@selector(showDetailAtIndex) forControlEvents:UIControlEventTouchUpInside];
    }
...
- (void)showDetailAtIndex{
   
    [_photoScrollView removeAllSubviews];
    [_photoScrollView removeFromSuperview];
    [self.view removeFromSuperview];
    [self photoViewSingleTap:nil];
    if ([self.delegate respondsToSelector:@selector(showDetailPic)]) {
        [self.delegate showDetailPic];
    }
}
#pragma 删除图片
- (void)deletePic{
    ...
    if ([self.delegate respondsToSelector:@selector(deletePictureAtIndex:)]) {
        [self.delegate deletePictureAtIndex:self.currentPhotoIndex];
    }
    if ([_photos count]==1){
    ...
    delete = YES;
    NSMutableArray * dataArray = [[NSMutableArray alloc]initWithArray:self.photos];
    MJPhoto * photo = dataArray[self.currentPhotoIndex];
    [_visiblePhotoViews removeObject:photo];
    [_reusablePhotoViews removeObject:photo];
    [dataArray removeObjectAtIndex:self.currentPhotoIndex];
    self.photos = [dataArray copy];
    _toolbar.photos = [dataArray copy];
...
- (void)setPhotos:(NSArray *)photos
{
    ...
    for (int i = 0; i<_photos.count; i++) {
        ...
        photo.index = i;
        if (i == _currentPhotoIndex &&!delete){
             photo.firstShow = YES;
        }
    }
}
...
- (void)showPhotos
{
	...
	long firstIndex = (int)floorf((CGRectGetMinX(visibleBounds)+kPadding*2) / CGRectGetWidth(visibleBounds));
	long lastIndex  = (int)floorf((CGRectGetMaxX(visibleBounds)-kPadding*2-1) / CGRectGetWidth(visibleBounds));
...
- (void)showPhotoViewAtIndex:(long)index
{
...
- (void)loadImageNearIndex:(long)index
{
//    if (index > 0) {
